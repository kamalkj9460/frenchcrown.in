...
//         document.dispatchEvent(new CustomEvent('theme:loading:start'));
        ...
        if($('.ProductForm__Item').length > 0 && !$('.sticky-bottom').hasClass('show')){
          ...
          }else if(meta.product.type == 'Suit'){
            var pant_size = '';
          	$('.ProductForm__Item').each(function(index){
        ...
        if(addToCartButton.getAttribute('disabled') == null){
          ...
          if($('.extra_pair:checked').length > 0){
            ...
            if(variantObj != undefined){
              ...
              console.log("extraPairOFPantProductObjParse==>", extraPairOFPantProductObjParse);
              extraPairOFPantProductObjParse.product.variants.forEach(variant => {
          ...
          dataLayer.push({
            ...
            'ecommerce': {
              'items': [
                {
                'item_name': $('.ptitle').val(),
                'item_id': $('.pid').val(),
                'sku' :$('.variant_list option:selected').attr('data-sku')
                }
              ]
            }
           });
  ...
  var Collapsible = function () {
    ...
    _createClass(Collapsible, [{ 
      ...
      value: function _attachListeners() {
        ...
        $('.CollectionInner__Sidebar .CollectionFilters .Collapsible:first .Collapsible__Button').attr('aria-expanded', 'true');
        $('.CollectionInner__Sidebar .CollectionFilters .Collapsible:first .Collapsible__Inner').css({'overflow': 'visible', 'height': 'auto'});
      }
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var CollectionSection = function () {
    function CollectionSection(element) {
      ...
      this.tagListingCollection(this.currentTags);
    ...
    _createClass(CollectionSection, [{
      key: 'tagListingCollection',
      value: function tagListingCollection(tagListing) {
        
        var _thisTag = this;
        
        //    Collection Filter tag bubble Set
        let tagListElement = document.querySelector('.CollectionInner__Products .selected_tag_list');
        tagListElement.innerHTML = "";

        __WEBPACK_IMPORTED_MODULE_3__helper_Dom__["default"].nodeListToArray(document.querySelectorAll('.CollectionInner__Sidebar .CollectionFilters [data-tag]')).forEach(function (item) {
          tagListing.forEach(tag => {
            if (tag === item.getAttribute('data-tag')) {
              let content = item.innerHTML;
              let tagListingDiv = document.createElement('div');
              tagListingDiv.className = "tag__Listing";
              tagListingDiv.innerHTML = content;

              let a = document.createElement('a'); 
              let link = document.createTextNode("X");
              a.appendChild(link);
              a.href = "javascript:void(0);";
              a.className = "tag__Listing__close";
              a.setAttribute('data-tag_name',content);
              a.setAttribute('data-action','close-tag');
              
              tagListingDiv.appendChild(a);

              tagListElement.appendChild(tagListingDiv);
            }
            
          });
        });
        
        //  tag bubble Close Functionality
        
        let closeTag = document.querySelectorAll('.tag__Listing__close');

        closeTag.forEach(item => {
          item.addEventListener('click', event => {
            let tagName = item.getAttribute('data-tag_name');

            document.querySelectorAll('.CollectionFilters [data-tag]').forEach(function (item) {
              if (tagName === item.innerHTML) {
                if (item.classList.contains('is-active')) {
                  _thisTag.temporaryTags.splice(_thisTag.temporaryTags.indexOf(item.getAttribute('data-tag')), 1); // Delete the tag if already active
                }
                _thisTag._applyTags();
              }
            });
      ...
      value: function _reloadProducts() {
        ...
        var filterElement = document.getElementsByClassName('CollectionToolbar__Item--filter');

        if (filterElement) {
          var filterElementCount = filterElement[0].querySelector('span');

          if (filterElementCount) {
            filterElement[0].removeChild(filterElementCount);
          }

          if (this.currentTags.length === 0) {
            filterElement[0].classList.add('Text--subdued');
          } else {
            filterElement[0].classList.remove('Text--subdued');
            filterElement[0].innerHTML += '<span class="Text--subdued">(' + this.currentTags.length + ')</span>';
          }
        }
        ...
        fetch(location.pathname + '?view=ajax&sort_by=' + this.currentSortBy, {
          ...
          response.text().then(function (content) {
            ...
            _this38.tagListingCollection(tagList);
    ...
    _createClass(ShopTheLookSection, [{
      ...
      value: function _createOuterCarousel() {
        ...
        __WEBPACK_IMPORTED_MODULE_2__helper_Dom__["default"].nodeListToArray(this.element.querySelectorAll('.ShopTheLook__ProductList')).forEach(function (item) {
          ...
          setTimeout(function(){ item.insertBefore(item.querySelector('.flickity-viewport'), item.querySelector('.ShopTheLook__ViewButton'));},1000);
        });
